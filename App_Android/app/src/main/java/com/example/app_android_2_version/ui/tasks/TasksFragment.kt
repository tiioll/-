...
class TasksFragment : Fragment(), TaskAdapter.Listener, TaskCalendarAdapter.Listener {
    ...
    var Months = listOf("Январь", "Февраль", "Март", "Апрель", "Май", "Июнь", "Июль", "Август", "Сентябрь", "Октябрь", "Ноябрь", "Декабрь")
    ...
    override fun onCreateView(
        ...
        binding.tasksSetDate.setOnClickListener {
            ...
            if (binding.tasksNewtask.getText().toString().trim() != "") {
                ...
                Toast.makeText(requireContext(), "Сначала введите задачу", Toast.LENGTH_SHORT).show()
            }
        }
        ...
        binding.tasksCreateTask.setOnClickListener {
            ...
            if (!TaskIsOnly(myRef)){
                Toast.makeText(requireContext(), "Задача с таким именем уже существует", Toast.LENGTH_SHORT).show()
            }
            ...
            if (Task != "" && TaskIsOnly(myRef)){
                ...
                currentUser?.uid?.let { uid ->
                    ...
                    userReference.child("tasks").child(Task).setValue(Taska(Task, TaskDate, GetWeekDay(TaskDate)))
                }

                updateCalendar()
                Toast.makeText(requireContext(), "Задача " + Task + " установлена до " + TaskDate, Toast.LENGTH_SHORT).show()
            }
        }
    ...
    private fun initCalendarRcView() = with(binding){
        ...
        daysOfMonths.layoutManager = LinearLayoutManager(requireContext(), LinearLayoutManager.HORIZONTAL, false)
    ...
    override fun onClick(taska: Taska) {
        ...
        updateCalendar()
        Toast.makeText(requireContext(), "Задача " + Task + ", установленная до " + taska.date + " удалена", Toast.LENGTH_SHORT).show()
    }
            ...
            currentUserUid?.let { uid -> FirebaseDatabase.getInstance().getReference("users").child(uid).addListenerForSingleValueEvent(object : ValueEventListener {
                override fun onDataChange(snapshot: DataSnapshot) {
                    ...
                    for (s in snap.children){
                        ...
                        if (taska != null && (taska.date?.let { dateCompare(it, activeDate) } == 0)){
                            list.add(taska)
                        }
                    }
